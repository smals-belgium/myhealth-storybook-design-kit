...
> The `Alert` is a additional ui component that displays a contextual message with an icon, a title, an optional details, action button and close button.
...
```html
<mh-alert [title]="'Pinned Alert'" [details]="'This alert cannot be dismissed.'" />
```
...
```html
<mh-alert [title]="'Closable Alert'" [dismissMode]="'closable'" [closeButtonAriaLabel]="'Close alert'" />
```
