...
Gives short additional informations about a topic within the page.<br />
_Variant_: collapse-expand
...
Prompted when user input is incorrect, forbidden, or the system fails to operate a task. The error message helps the user by giving him the feedback about what error the encountered.
...
- When a **dialog** is used for alert, then it appears in front of web page content to provide information or ask for a decision. Dialog disable all page functionality when they appear, and remain on screen until confirmed, dismissed, or a required action has been taken.
