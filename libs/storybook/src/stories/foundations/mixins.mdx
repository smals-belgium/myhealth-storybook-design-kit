...
The `core($use-material)` mixin is the base of the design kit.

This mixin allows the following variables to be passed: `$use-material: boolean`.

- `$use-material` is a boolean that dictates if `core()` **includes Angular Material** theme in the design kit or not.
- `$symbols-font-url` is an `url()` that specifies the path to the Material Symbols font file.

This mixin in turn calls following mixins:

- [use-font-open-sans()](#use-font-open-sans)
- [use-font-material-symbols()](#use-font-material-symbols)
...
The `use-font-open-sans()` includes the Open Sans font into SCSS `@font-face`.

### use-font-material-symbols

The `use-font-material-symbols($symbols-font-url)` includes the Open Sans font into SCSS `@font-face`.

This mixin allows the following variables to be passed: `$symbols-font-url: url(string)`.
...
The `override-mat-select-arrow($icon)` mixin is used to override the default Material dropdown arrow in a select.

This mixin allows the following variable to be passed: `$icon: string`.
