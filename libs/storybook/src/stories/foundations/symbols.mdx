...
@include mh.core($use-material: true, $symbols-font-url: url('assets/symbols.ttf'));
```

Make sure `$symbols-font-url` points to the location of the TTF file in your application.

Alternatively, you can also pass it directly to the symbols mixin if you're not using the `mh.core()` mixin:

```css
@include mh-fonts.use-font-material-symbols($symbols-font-url: url('assets/symbols.ttf'));
```
